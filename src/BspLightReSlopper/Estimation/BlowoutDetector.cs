...
namespace BspLightReSlopper.Estimation
{
    ...
    public sealed class BlowoutDetector
    {
        ...
        {
            /// <summary>A texel is <b>saturated</b> when its max RGB channel reaches this
            /// threshold. q3 lightmaps clip at 1.0 (255/255); 0.95 catches the
            /// near-clip region too where the s-curve flattens.</summary>
            public float SaturationThreshold { get; init; } = 0.95f;
        ...
        {
            /// <summary>Per-sample mask: true = sample is in a blown cluster and should NOT
            /// participate in the standard LSQ. Same length as the input sample list.</summary>
        ...
        public static Result Detect(IReadOnlyList<TexelSample> samples, Options? options = null, Logger? log = null)
        {
            ...
            if (n == 0) return new Result { BlownMask = blown };
            ...
            for (int cid = 0; cid < clusters.Count; cid++)
            {
                ...
                if (flat)
                {
                    ...
                    foreach (int idx in members) { blown[idx] = true; blownCount++; }
                }
            }
            ...
            for (int cid = 0; cid < clusters.Count; cid++)
            {
                ...
                foreach (int idx in members)
                {
                    foreach (var (dx, dy) in Neighbours)
                    {
                        if (index.TryGetValue((samples[idx].AtlasIndex, samples[idx].AtlasX + dx, samples[idx].AtlasY + dy, samples[idx].SurfaceIndex, samples[idx].Stage), out int j))
                        {
                            if (blown[j]) continue; // interior neighbour
            ...
            log?.Info($"  blowout: {saturatedCount} saturated texels, {blownCount} blown ({(blownCount / (float)Math.Max(1, n)):P1} of all samples), {clusters.Count} clusters, {pointCandidates.Count} point candidates, {sunCandidates.Count} sun candidates");
